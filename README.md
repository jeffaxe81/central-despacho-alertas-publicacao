...
Na MUE-009, o dispatcher passou a registrar o resultado da publicação no canal shadow com o evento estruturado `eventbus.canonical_shadow_published`. O log inclui `correlationId`, `eventId`, tipo canônico, indicador de simulação, resultado `equivalent` e as contagens `delivered`/`failed`; usa nível `info` quando não há falhas e `warn` quando algum assinante falha. Essa falha permanece observacional e não altera a resposta `202` do despacho.

Na MUE-010, o canal shadow passou a manter um snapshot diagnóstico cumulativo somente em memória. A consulta `getCanonicalShadowDiagnosticSnapshot()` devolve uma cópia somente de leitura com `publications`, `delivered`, `failed`, `equivalent` e `divergent`. O reset do canal de teste também reinicializa esses contadores. O diagnóstico não cria persistência nem atravessa processos ou réplicas.

Na MUE-011, foi criada a fronteira interna `readCanonicalShadowDiagnosticSnapshot()`. Ela exige `isTestMode: true`, rejeita consultas fora do modo teste e devolve uma cópia defensiva do snapshot da MUE-010. A fronteira não é publicada como rota HTTP ou procedimento tRPC e permanece isolada do fluxo produtivo.

Na MUE-012, foi criado o consumidor interno `createCanonicalShadowDiagnosticReport()`. Ele usa exclusivamente a fronteira da MUE-011 para classificar o diagnóstico como `idle`, `healthy` ou `attention` e calcular taxas de sucesso de entrega e equivalência. O consumidor permanece somente leitura, exige modo teste e não cria qualquer exposição HTTP/tRPC ou persistência.

Na MUE-013, foi adicionada a política interna `evaluateCanonicalShadowDiagnosticPolicy()`. Ela recebe do chamador limiares de sucesso de entrega, equivalência, falhas e divergências, valida esses valores e classifica o resultado como `idle`, `healthy` ou `warning`, incluindo uma lista objetiva das violações encontradas. Nenhum limiar de negócio é fixado pela plataforma, e a política continua restrita ao modo teste.

Na MUE-014, foi adicionado o comparador puro `compareCanonicalShadowDiagnosticTrend()`. Ele recebe duas avaliações consecutivas da política da MUE-013 e compara taxas de entrega/equivalência, falhas, divergências e quantidade de violações. A tendência é `improving` quando há melhora sem regressão, `degrading` quando há regressão sem melhora e `stable` quando as métricas não mudam, há sinais mistos ou falta base comparável por estado `idle`. O comparador não mantém histórico próprio nem atribui pesos arbitrários às métricas.

Na MUE-015, foi adicionado o rastreador interno `trackCanonicalShadowDiagnostic()`. Ele mantém somente a última avaliação da política em memória: a primeira leitura estabelece a baseline e não produz tendência; a partir da segunda, a leitura anterior é enviada ao comparador da MUE-014 e a baseline é substituída pela avaliação atual. O reset do rastreador remove apenas essa baseline interna, sem reinicializar os contadores do canal shadow. Não há persistência nem histórico multi-amostra.
